...
export default class KlineChart extends Chart {
  ...
  drawChart() {
    const maPoints = [[], [], [], []];
    const color = ['#FFD11E', '#F77BFC', '#39C2FD', '#B7B7B7'];
    ...
    this.data.forEach((eachData, index) => {
      ...
      if (currentLabel !== lastLabel) {
        ...
        if ((index - lastDrawIndex) * pixelPerWithSeparator > 100) {
          this.drawXAxisGridLine((pixelPerWithSeparator * index) + halfPixelPer, currentLabel);
          lastDrawIndex = index;
        }
      }
