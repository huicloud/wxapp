...
import { DataStore, noop } from '../utils/util';
...
export default class DataProvider {
  ...
  getKline(params, withMA = true) {
    const klineQuery = this.klineDataStore.query(
  ...
  subscribeKline(params, callback = noop) {
    const request = this.klineDataStore.subscribe(
  ...
  getMA(params) {
    return this.maDataStore.query(Object.assign({}, this.params, params))
  ...
  _initMinCache(timeInfo) {
    ...
    if (times && times.length > 0) {
      ...
      // FIXME 默认包含集合进价的数据
      const prefixMinute = timeInfo.JiHeJingJiaDianShu || 0;
  ...
  subscribeMin(callback = noop) {
    ...
    const request = this.minDataStore.subscribe(this.params, (data) => {
