...
var webpackConfig = {
	...
	output: {
		filename: '[name].js',
		libraryTarget: 'umd', // var和amd加载都不正确
	},
	module: {
		preLoaders: [
	...
	externals: [
		function(context, request, callback) {
			...
			// 在src目录中互相引用保持require语句
			if(inSrc(context) && inSrc(request, context)) {
...
function inSrc(filePath, relativePath) {
	var absolutePath;
	try {
		absolutePath = require.resolve(filePath);
	} catch (e) {
		absolutePath = path.resolve(relativePath || './', filePath);
	}
	return path.relative('./', absolutePath).indexOf('src/') === 0;
}
